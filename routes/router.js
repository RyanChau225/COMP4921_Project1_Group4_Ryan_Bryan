 ...
 // GETS ALL MEDIA (NOT JUST THE USERS, NEED TO SWAP TO "showpets" version)
  ...
  router.post('/addMedia', async (req, res) => {
	try {
	  ...
	  let image_id = req.body.image_id; // Extract image_id from the request
	  let active = req.body.active === 'true';
	  let url = 'https://example.com';
	  let shortURL = 'https://example.com/short';
	  ...
	  const schema = Joi.object({
		...
		image_id: Joi.when('media_type', {
		  is: 'image',
		  then: Joi.string().alphanum().min(24).max(24).required(), // Define validation for image_id when media_type is 'image'
		  otherwise: Joi.optional()
		}),
	  ...
	  const validationResult = schema.validate({
		...
		original_link,
		text_content,
		image_id, // Include image_id in the validation
		active,
		url,
		shortURL,
		created: new Date(),
	  ...
	  if (media_type === 'links') {
		...
		document.image_id = image_id; // Add image_id to the document when media_type is 'image'
	  }
